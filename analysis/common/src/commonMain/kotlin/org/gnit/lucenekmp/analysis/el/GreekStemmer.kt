...
/**
 * A stemmer for Greek words, according to: <i>Development of a Stemmer for the Greek Language.</i>
 * Georgios Ntais
 *
 * <p>NOTE: Input is expected to be casefolded for Greek (including folding of final sigma to
 * sigma), and with diacritics removed. This can be achieved with either {@link
 * GreekLowerCaseFilter} or ICUFoldingFilter.
 *
 * @lucene.experimental
 */
internal class GreekStemmer {

  /**
   * Stems a word contained in a leading portion of a char[] array. The word is passed through a
   * number of rules that modify its length.
   *
  ...
  fun stem(s: CharArray, len: Int): Int {
    ...
    len = rule20(s, len)
    // "long list"
    if (len == origLen) len = rule21(s, len)
    return rule22(s, len)
  }

  private fun rule0(s: CharArray, len: Int): Int {
    val len = len
  ...
  private fun rule1(s: CharArray, len: Int): Int {
    var len = len
    if (len > 4 && (endsWith(s, len, "αδεσ") || endsWith(s, len, "αδων"))) {
      ...
      if (!(endsWith(s, len, "οκ")
          ...
          || endsWith(s, len, "κυρ")
          || endsWith(s, len, "θει")
          || endsWith(s, len, "πεθερ"))) len += 2 // add back -αδ
    }
    return len
  }

  private fun rule2(s: CharArray, len: Int): Int {
    var len = len
    if (len > 4 && (endsWith(s, len, "εδεσ") || endsWith(s, len, "εδων"))) {
      ...
      if (endsWith(s, len, "οπ")
          ...
          || endsWith(s, len, "μιλ")) len += 2 // add back -εδ
    }
    return len
  }

  private fun rule3(s: CharArray, len: Int): Int {
    var len = len
    if (len > 5 && (endsWith(s, len, "ουδεσ") || endsWith(s, len, "ουδων"))) {
      ...
      if (endsWith(s, len, "αρκ")
          ...
          || endsWith(s, len, "σπ")
          || endsWith(s, len, "τραγ")
          || endsWith(s, len, "φε")) len += 3 // add back -ουδ
    }
    return len
  }

    private val exc4: CharArraySet =
      CharArraySet(mutableListOf("θ", "δ", "ελ", "γαλ", "ν", "π", "ιδ", "παρ"), false)
  private fun rule4(s: CharArray, len: Int): Int {
    var len = len
  ...
  private fun rule5(s: CharArray, len: Int): Int {
    var len = len
    ...
    private val exc6: CharArraySet =
      CharArraySet(mutableListOf("αλ", "αδ", "ενδ", "αμαν", "αμμοχαλ", "ηθ", "ανηθ", "αντιδ", "φυσ", "βρωμ", "γερ",
              "εξωδ", "καλπ", "καλλιν", "καταδ", "μουλ", "μπαν", "μπαγιατ", "μπολ", "μποσ", "νιτ",
              "ξικ", "συνομηλ", "πετσ", "πιτσ", "πικαντ", "πλιατσ", "ποστελν", "πρωτοδ", "σερτ",
              "συναδ", "τσαμ", "υποδ", "φιλον", "φυλοδ", "χασ"), false)
  private fun rule6(s: CharArray, len: Int): Int {
    var len = len
    var removed = false
    ...
    if (removed) {
      if (endsWithVowel(s, len) || exc6.contains(s, 0, len)) len += 2 // add back -ικ
    }
    return len
  }

    private val exc7: CharArraySet =
      CharArraySet(mutableListOf("αναπ", "αποθ", "αποκ", "αποστ", "βουβ", "ξεθ", "ουλ", "πεθ", "πικρ", "ποτ", "σιχ",
              "χ"), false)
  private fun rule7(s: CharArray, len: Int): Int {
    var len = len
    if (len == 5 && endsWith(s, len, "αγαμε")) return len - 1
    if (len > 7 && endsWith(s, len, "ηθηκαμε")) len -= 7
    else if (len > 6 && endsWith(s, len, "ουσαμε")) len -= 6
    else if (len > 5
        && (endsWith(s, len, "αγαμε") || endsWith(s, len, "ησαμε") || endsWith(s, len, "ηκαμε")))
      len -= 5
    if (len > 3 && endsWith(s, len, "αμε")) {
      len -= 3
      if (exc7.contains(s, 0, len)) len += 2 // add back -αμ
    }

    return len
  }

    private val exc8a: CharArraySet =
      CharArraySet(mutableListOf("τρ", "τσ"), false)
    private val exc8b: CharArraySet =
      CharArraySet(mutableListOf("βετερ",
              "βουλκ",
              "βραχμ",
              "γ",
              "δραδουμ",
              ...
              "χαμηλοδαπ",
              "ψηλοταβ"), false)
  private fun rule8(s: CharArray, len: Int): Int {
    var len = len
    var removed = false
    ...
    private val exc9: CharArraySet =
      CharArraySet(mutableListOf("αβαρ", "βεν", "εναρ", "αβρ", "αδ", "αθ", "αν", "απλ", "βαρον", "ντρ", "σκ", "κοπ",
              "μπορ", "νιφ", "παγ", "παρακαλ", "σερπ", "σκελ", "συρφ", "τοκ", "υ", "δ", "εμ",
              "θαρρ", "θ"), false)
  private fun rule9(s: CharArray, len: Int): Int {
    var len = len
  ...
  private fun rule10(s: CharArray, len: Int): Int {
    var len = len
  ...
  private fun rule11(s: CharArray, len: Int): Int {
    var len = len
    ...
    private val exc12a: CharArraySet =
      CharArraySet(mutableListOf("π", "απ", "συμπ", "ασυμπ", "ακαταπ", "αμεταμφ"), false)
    private val exc12b: CharArraySet =
      CharArraySet(mutableListOf("αλ", "αρ", "εκτελ", "ζ", "μ", "ξ", "παρακαλ", "αρ", "προ", "νισ"), false)
  private fun rule12(s: CharArray, len: Int): Int {
    var len = len
    ...
    if (len > 4 && endsWith(s, len, "εστε")) {
      len -= 4
      if (exc12b.contains(s, 0, len)) len += 3 // add back -εστ
    }

    return len
  }

    private val exc13: CharArraySet =
      CharArraySet(mutableListOf("διαθ", "θ", "παρακαταθ", "προσθ", "συνθ"), false)
  private fun rule13(s: CharArray, len: Int): Int {
    var len = len
    ...
    private val exc14: CharArraySet =
      CharArraySet(mutableListOf("φαρμακ",
              "χαδ",
              "αγκ",
              "αναρρ",
              "βρομ",
              ...
              "πλε",
              "τσα"), false)
  private fun rule14(s: CharArray, len: Int): Int {
    var len = len
    var removed = false
    ...
    private val exc15a: CharArraySet =
      CharArraySet(mutableListOf("αβαστ",
              "πολυφ",
              "αδηφ",
              "παμφ",
              "ρ",
              ...
              "τρ",
              "μ"), false)
    private val exc15b: CharArraySet =
      CharArraySet(mutableListOf("ψοφ", "ναυλοχ"), false)
  private fun rule15(s: CharArray, len: Int): Int {
    var len = len
    var removed = false
    ...
    if (removed) {
      val cond1 =
          exc15a.contains(s, 0, len)
              ...
              || endsWith(s, len, "λοχ")
              || endsWith(s, len, "σμην")
      val cond2 = exc15b.contains(s, 0, len) || endsWith(s, len, "κολλ")
    ...
    private val exc16: CharArraySet =
      CharArraySet(mutableListOf("ν", "χερσον", "δωδεκαν", "ερημον", "μεγαλον", "επταν"), false)
  private fun rule16(s: CharArray, len: Int): Int {
    var len = len
    var removed = false
    ...
    if (removed && exc16.contains(s, 0, len)) len += 2 // add back -ησ

    return len
  }

    private val exc17: CharArraySet =
      CharArraySet(mutableListOf("ασβ", "σβ", "αχρ", "χρ", "απλ", "αειμν", "δυσχρ", "ευχρ", "κοινοχρ", "παλιμψ"), false)
  private fun rule17(s: CharArray, len: Int): Int {
    var len = len
    ...
    private val exc18: CharArraySet =
      CharArraySet(mutableListOf("ν", "ρ", "σπι", "στραβομουτσ", "κακομουτσ", "εξων"), false)
  private fun rule18(s: CharArray, len: Int): Int {
    var len = len
    var removed = false
    ...
    private val exc19: CharArraySet =
      CharArraySet(mutableListOf("παρασουσ", "φ", "χ", "ωριοπλ", "αζ", "αλλοσουσ", "ασουσ"), false)
  private fun rule19(s: CharArray, len: Int): Int {
    var len = len
    var removed = false
  ...
  private fun rule20(s: CharArray, len: Int): Int {
    var len = len
  ...
  private fun rule21(s: CharArray, len: Int): Int {
    val len = len
  ...
  private fun rule22(s: CharArray, len: Int): Int {
    val len = len
    ...
    if (endsWith(s, len, "οτερ")
        ...
        || endsWith(s, len, "υτατ")
        || endsWith(s, len, "ωτερ")
        || endsWith(s, len, "ωτατ")) return len - 4
    return len
  }

  /**
   * Checks if the word contained in the leading portion of char[] array , ends with the suffix
   * given as parameter.
   *
   ...
   *     false otherwise.
   */
  private fun endsWithVowel(s: CharArray, len: Int): Boolean {
   ...
   *     false otherwise.
   */
  private fun endsWithVowelNoY(s: CharArray, len: Int): Boolean {
    if (len == 0) return false
    return when (s[len - 1]) {
      'α', 'ε', 'η', 'ι', 'ο', 'ω' -> true
      else -> false
