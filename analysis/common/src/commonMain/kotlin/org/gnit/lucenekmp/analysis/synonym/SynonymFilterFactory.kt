 ...
 * @deprecated Use [SynonymGraphFilterFactory] instead, but be sure to also use
 * [org.gnit.lucenekmp.analysis.core.FlattenGraphFilterFactory] at index time (not at search time) as well.
 * @since 3.1
 * @lucene.spi {@value #NAME}
 */
@Deprecated("Use SynonymGraphFilterFactory instead")
class SynonymFilterFactory : TokenFilterFactory, ResourceLoaderAware {
    companion object {
        /** SPI name */
        const val NAME = "synonym"
    }
    ...
    override fun create(input: TokenStream): TokenStream {
        ...
        return if (map.fst == null) input else SynonymFilter(input, map, ignoreCase)
    }
