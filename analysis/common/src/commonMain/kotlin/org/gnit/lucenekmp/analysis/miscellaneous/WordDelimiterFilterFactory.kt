 ...
 *
 * @deprecated Use [WordDelimiterGraphFilterFactory] instead.
 * @since 3.1
 * @lucene.spi [NAME]
 */
@Deprecated("Use WordDelimiterGraphFilterFactory instead")
class WordDelimiterFilterFactory : TokenFilterFactory, ResourceLoaderAware {
    ...
    constructor(args: MutableMap<String, String>) : super(args) {
        ...
        if (getInt(args, "generateWordParts", 1) != 0) {
            flags = flags or WordDelimiterFilter.GENERATE_WORD_PARTS
        }
        if (getInt(args, "generateNumberParts", 1) != 0) {
            flags = flags or WordDelimiterFilter.GENERATE_NUMBER_PARTS
        }
        if (getInt(args, "catenateWords", 0) != 0) {
            flags = flags or WordDelimiterFilter.CATENATE_WORDS
        }
        if (getInt(args, "catenateNumbers", 0) != 0) {
            flags = flags or WordDelimiterFilter.CATENATE_NUMBERS
        }
        if (getInt(args, "catenateAll", 0) != 0) {
            flags = flags or WordDelimiterFilter.CATENATE_ALL
        }
        if (getInt(args, "splitOnCaseChange", 1) != 0) {
            flags = flags or WordDelimiterFilter.SPLIT_ON_CASE_CHANGE
        }
        if (getInt(args, "splitOnNumerics", 1) != 0) {
            flags = flags or WordDelimiterFilter.SPLIT_ON_NUMERICS
        }
        if (getInt(args, "preserveOriginal", 0) != 0) {
            flags = flags or WordDelimiterFilter.PRESERVE_ORIGINAL
        }
        if (getInt(args, "stemEnglishPossessive", 1) != 0) {
            flags = flags or WordDelimiterFilter.STEM_ENGLISH_POSSESSIVE
        }
    ...
    private fun parseType(s: String): Byte? {
        return when (s) {
            "LOWER" -> WordDelimiterFilter.LOWER.toByte()
            "UPPER" -> WordDelimiterFilter.UPPER.toByte()
            "ALPHA" -> WordDelimiterFilter.ALPHA.toByte()
            "DIGIT" -> WordDelimiterFilter.DIGIT.toByte()
            "ALPHANUM" -> WordDelimiterFilter.ALPHANUM.toByte()
            "SUBWORD_DELIM" -> WordDelimiterFilter.SUBWORD_DELIM.toByte()
            else -> null
        }
    }
