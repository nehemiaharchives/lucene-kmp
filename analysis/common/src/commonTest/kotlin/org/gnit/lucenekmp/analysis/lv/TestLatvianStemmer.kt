...
/** Basic tests for [LatvianStemmer] */
class TestLatvianStemmer : BaseTokenStreamTestCase() {
    ...
    fun setUp() {
        a = object : Analyzer() {
            override fun createComponents(fieldName: String): TokenStreamComponents {
                ...
                return TokenStreamComponents(tokenizer, LatvianStemFilter(tokenizer))
            }
        }
    }
  ...
  /**
   * decl II nouns with (s,t) -&gt š and (d,z) -&gt ž palatalization will generally conflate to
   * two stems due to the ambiguity (plural and singular).
   */
    @Test
    @Throws(IOException::class)
    fun testNouns2() {
  ...
  /**
   * Genitive plural forms with (s,t) -&gt š and (d,z) -&gt ž will not conflate due to ambiguity.
   */
    @Test
    @Throws(IOException::class)
    fun testNouns5() {
  ...
  /** Note: we intentionally don't handle the ambiguous (s,t) -&gt š and (d,z) -&gt ž */
    @Test
    @Throws(IOException::class)
    fun testPalatalization() {
  ...
  /** Test some length restrictions, we require a 3+ char stem, with at least one vowel. */
    @Test
    @Throws(IOException::class)
    fun testLength() {
