...
/** Simple tests for [NorwegianMinimalStemFilter]. */
class TestNorwegianMinimalStemFilter : BaseTokenStreamTestCase() {
    ...
    fun setUp() {
        analyzer = object : Analyzer() {
            override fun createComponents(fieldName: String): TokenStreamComponents {
                ...
                return TokenStreamComponents(source, NorwegianMinimalStemFilter(source, BOKMAAL))
            }
        }
    }
    ...
    @Throws(IOException::class)
    fun testKeyword() {
        val exclusionSet = CharArraySet(mutableSetOf<Any>("sekretæren"), false)
        val a = object : Analyzer() {
            override fun createComponents(fieldName: String): TokenStreamComponents {
                val source: Tokenizer = MockTokenizer(MockTokenizer.WHITESPACE, false)
                val sink: TokenStream = SetKeywordMarkerFilter(source, exclusionSet)
                return TokenStreamComponents(source, NorwegianMinimalStemFilter(sink))
            }
        }
    ...
    fun testEmptyTerm() {
        val a = object : Analyzer() {
            override fun createComponents(fieldName: String): TokenStreamComponents {
                ...
                return TokenStreamComponents(tokenizer, NorwegianMinimalStemFilter(tokenizer))
            }
        }
        checkOneTerm(a, "", "")
        a.close()
