...
class LongRangeDocValuesField(
    ...
    val min: LongArray,
    val max: LongArray,
) : BinaryRangeDocValuesField(field, LongRange.encode(min, max), min.size, LongRange.BYTES) {
    /** Sole constructor. */
    init {
        checkArgs(min, max)
    }
    ...
    companion object {
        private fun newSlowRangeQuery(
            ...
            checkArgs(min, max)
            return LongRangeSlowRangeQuery(field, min, max, queryType)
        }
         ...
         */
        fun newSlowIntersectsQuery(field: String, min: LongArray, max: LongArray): Query {
