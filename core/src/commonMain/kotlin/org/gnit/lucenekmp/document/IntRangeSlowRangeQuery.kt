...
class IntRangeSlowRangeQuery(
    ...
    private val min: IntArray,
    private val max: IntArray,
    queryType: RangeFieldQuery.QueryType,
) : BinaryRangeFieldRangeQuery(field, encodeRanges(min, max), IntRange.BYTES, min.size, queryType) {
    override fun equals(other: Any?): Boolean {
        if (sameClassAs(other).not()) {
            return false
        }
        other as IntRangeSlowRangeQuery
    ...
    companion object {
        private fun encodeRanges(min: IntArray, max: IntArray): ByteArray {
            val result = ByteArray(2 * IntRange.BYTES * min.size)
