...
class DoubleRangeSlowRangeQuery(
...
) : BinaryRangeFieldRangeQuery(field, encodeRanges(min, max), DoubleRange.BYTES, min.size, queryType) {
    override fun equals(other: Any?): Boolean {
        if (sameClassAs(other).not()) {
            return false
        }
        other as DoubleRangeSlowRangeQuery
    ...
    companion object {
        private fun encodeRanges(min: DoubleArray, max: DoubleArray): ByteArray {
            val result = ByteArray(2 * DoubleRange.BYTES * min.size)
