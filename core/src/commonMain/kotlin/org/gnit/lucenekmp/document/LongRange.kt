 ...
 *
 */
class LongRange(name: String, min: LongArray, max: LongArray) : Field(name, getType(min.size)) {
    /**
     ...
     *
     * @param min array of min values. (accepts `Long.MIN_VALUE`)
     * @param max array of max values. (accepts `Long.MAX_VALUE`)
     * @throws IllegalArgumentException if `min` or `max` is invalid
     */
    fun setRangeValues(min: LongArray, max: LongArray) {
        checkArgs(min, max)
        require(!(min.size * 2 != type.pointDimensionCount() || max.size * 2 != type.pointDimensionCount())) {
            ("field (name="
                    + name
                    + ") uses "
    ...
    companion object {
        ...
        const val BYTES: Int = Long.SIZE_BYTES

        /** set the field type  */
        private fun getType(dimensions: Int): FieldType {
            require(dimensions <= 4) { "LongRange does not support greater than 4 dimensions" }
        ...
        private fun checkArgs(min: LongArray, max: LongArray) {
            ...
            require(min.size <= 4) { "LongRange does not support greater than 4 dimensions" }
        }

        /** Encodes the min, max ranges into a byte array  */
        fun encode(min: LongArray, max: LongArray): ByteArray {
            checkArgs(min, max)
         ...
         */
        fun verifyAndEncode(min: LongArray, max: LongArray, bytes: ByteArray) {
            var d = 0
            var i = 0
        ...
        private fun encode(value: Long, bytes: ByteArray, offset: Int) {
            NumericUtils.longToSortableBytes(value, bytes, offset)
        }

        /** decodes the min value (for the defined dimension) from the encoded input byte array  */
        fun decodeMin(b: ByteArray, dimension: Int): Long {
        ...
        fun decodeMax(b: ByteArray, dimension: Int): Long {
         ...
         * invalid
         */
        fun newIntersectsQuery(field: String, min: LongArray, max: LongArray): Query {
         ...
         * invalid
         */
        fun newContainsQuery(field: String, min: LongArray, max: LongArray): Query {
         ...
         * invalid
         */
        fun newWithinQuery(field: String, min: LongArray, max: LongArray): Query {
         ...
         * invalid
         */
        fun newCrossesQuery(field: String, min: LongArray, max: LongArray): Query {
