...
class LongRangeSlowRangeQuery(
    ...
    private val min: LongArray,
    private val max: LongArray,
    queryType: RangeFieldQuery.QueryType,
) : BinaryRangeFieldRangeQuery(field, encodeRanges(min, max), LongRange.BYTES, min.size, queryType) {
    override fun equals(other: Any?): Boolean {
        if (sameClassAs(other).not()) {
            return false
        }
        other as LongRangeSlowRangeQuery
    ...
    companion object {
        private fun encodeRanges(min: LongArray, max: LongArray): ByteArray {
            val result = ByteArray(2 * LongRange.BYTES * min.size)
