 ...
 *
 * Only one read operation upon a readable channel may be in progress at any given time. If one
 ...
 * @since 1.4
 */
@Ported(from = "java.nio.channels.ReadableByteChannel")
interface ReadableByteChannel : Channel {
     ...
     *
     * Suppose that a byte sequence of length n is read, where 0 <= n <= r. This byte sequence will
     * be transferred into the buffer so that the first byte in the sequence is at index p and the
     * last byte is at index p + n - 1, where p is the buffer's position at the moment this method is
     ...
     * from the socket's input buffer; similarly, a file channel cannot read any more bytes than
     * remain in the file. It is guaranteed, however, that if a channel is in blocking mode and there
     * is at least one byte remaining in the buffer then this method will block until at least one
     * byte is read.
     *
