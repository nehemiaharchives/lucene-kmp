...
/**
 * [MutableValue] implementation of type `double`. When mutating instances of this
 * object, the caller is responsible for ensuring that any instance where `exists` is set
 * to `false` must also `value` set to `0.0D` for proper operation.
 */
class MutableValueDouble : MutableValue() {
    ...
    override fun copy(source: MutableValue) {
        val s = source as MutableValueDouble
        value = s.value
        exists = s.exists
    }

    override fun duplicate(): MutableValue {
        val v = MutableValueDouble()
        v.value = this.value
    ...
    override fun equalsSameType(other: Any?): Boolean {
        assert(exists || 0.0 == value)
        val b = other as MutableValueDouble
    ...
    override fun hashCode(): Int {
        assert(exists || 0.0 == value)
        val x = value.toBits()
        return x.toInt() + (x ushr 32).toInt()
    }
}
