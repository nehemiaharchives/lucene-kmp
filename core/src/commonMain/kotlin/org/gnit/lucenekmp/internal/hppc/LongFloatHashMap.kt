...
/**
 * A hash map of `int` to `double`, implemented using open addressing with linear
 ...
 *
 *
 * github: https://github.com/carrotsearch/hppc release 0.10f
 *
 * @lucene.internal
 */
@OptIn(ExperimentalAtomicApi::class)
open class LongFloatHashMap(expectedElements: Int, loadFactor: Float = DEFAULT_LOAD_FACTOR.toFloat()) :
    Iterable<LongFloatHashMap.LongFloatCursor>, Accountable, Cloneable<LongFloatHashMap> {
    ...
    protected var loadFactor: Float = 0f
    ...
    constructor(map: LongFloatHashMap) : this(map.size()) {
        putAll(map)
    }

    fun put(key: Long, value: Float): Float {
        require(assigned < mask + 1)

        val mask = this.mask
        if (((key) == 0L)) {
            val previousValue = if (hasEmptyKey) values!![mask + 1] else 0f
            hasEmptyKey = true
            values!![mask + 1] = value
    ...
    fun putAll(iterable: Iterable<LongFloatCursor>): Int {
        val count = size()
        for (c in iterable) {
            put(c.key, c.value)
        }
     ...
     * in the map.
     */
    fun putIfAbsent(key: Long, value: Float): Boolean {
     ...
     */
    fun putOrAdd(key: Long, putValue: Float, incrementValue: Float): Float {
     ...
     * @return Returns the current value associated with `key` (after changes).
     */
    fun addTo(key: Long, incrementValue: Float): Float {
        return putOrAdd(key, incrementValue, incrementValue)
    }

    fun remove(key: Long): Float {
        val mask = this.mask
        if (((key) == 0L)) {
            if (!hasEmptyKey) {
                return 0f
    ...
    fun get(key: Long): Float {
        if (((key) == 0L)) {
            return if (hasEmptyKey) values!![mask + 1] else 0f
        } else {
            val keys = this.keys
            val mask = this.mask
            var slot = hashKey(key) and mask
    ...
    fun getOrDefault(key: Long, defaultValue: Float): Float {
        if (((key) == 0L)) {
    ...
    fun indexReplace(index: Int, newValue: Float): Float {
        require(index >= 0) { "The index must point at an existing key." }
        require(index <= mask || (index == mask + 1 && hasEmptyKey))
    ...
    fun indexInsert(index: Int, key: Long, value: Float) {
        var index = index
    ...
    fun indexRemove(index: Int): Float {
        require(index >= 0) { "The index must point at an existing key." }
        require(index <= mask || (index == mask + 1 && hasEmptyKey))
    ...
    protected fun equalElements(other: LongFloatHashMap): Boolean {
        if (other.size() != size()) {
            return false
        }
    ...
    fun ensureCapacity(expectedElements: Int) {
        if (expectedElements > resizeAt || keys == null) {
            ...
            allocateBuffers(minBufferSize(expectedElements, loadFactor.toDouble()))
    ...
    private inner class EntryIterator : AbstractIterator<LongFloatCursor?>() {
        private val cursor: LongFloatCursor
        ...
        override fun fetch(): LongFloatCursor? {
            val mask = this@LongFloatHashMap.mask
            while (index <= mask) {
                val existing: Long
                index++
    ...
    override fun iterator(): Iterator<LongFloatCursor> {
        return EntryIterator() as Iterator<LongFloatCursor>
    }
    ...
    inner class KeysContainer : Iterable<LongCursor> {
        override fun iterator(): Iterator<LongCursor> {
            return KeysIterator() as Iterator<LongCursor>
        }

        fun size(): Int {
            return this@LongFloatHashMap.size()
        }
     ...
     */
    fun values(): FloatContainer {
        return ValuesContainer()
    }

    /** A view over the set of values of this map.  */
    private inner class ValuesContainer : FloatContainer() {
        override fun iterator(): Iterator<FloatCursor> {
            return ValuesIterator() as Iterator<FloatCursor>
        }
    }

    /** FloatCursor iterable with size and toArray function implemented  */
    abstract inner class FloatContainer : Iterable<FloatCursor> {
        fun size(): Int {
            return this@LongFloatHashMap.size()
        }
    ...
    private inner class ValuesIterator : AbstractIterator<FloatCursor?>() {
    ...
    override fun clone(): LongFloatHashMap {
        val cloned = LongFloatHashMap()
     ...
     * factors.
     */
    protected fun verifyLoadFactor(loadFactor: Float): Float {
        checkLoadFactor(loadFactor.toDouble(), MIN_LOAD_FACTOR.toDouble(), MAX_LOAD_FACTOR.toDouble())
        return loadFactor
    }

    /** Rehash from old buffers to new buffers.  */
    protected fun rehash(fromKeys: LongArray, fromValues: FloatArray) {
    ...
    protected fun allocateBuffers(arraySize: Int) {
        ...
        try {
            ...
            this.values = (FloatArray(arraySize + emptyElementSlot))
        } catch (e: Error) {
            this.keys = prevKeys
        ...
        this.resizeAt = expandAtCount(arraySize, loadFactor.toDouble())
     ...
     */
    protected fun allocateThenInsertThenRehash(slot: Int, pendingKey: Long, pendingValue: Float) {
        require(assigned == resizeAt && ((keys!![slot]) == 0L) && ((pendingKey) != 0L))

        // Try to allocate new buffers first. If we OOM, we leave in a consistent state.
        val prevKeys = this.keys
        val prevValues = this.values
        allocateBuffers(nextBufferSize(mask + 1, size(), loadFactor.toDouble()))
    ...
    companion object {
        private val BASE_RAM_BYTES_USED: Long = RamUsageEstimator.shallowSizeOfInstance(LongFloatHashMap::class)

        /** Creates a hash map from two index-aligned arrays of key-value pairs.  */
        fun from(keys: LongArray, values: FloatArray): LongFloatHashMap {
            require(keys.size == values.size) { "Arrays of keys and values must have an identical length." }

            val map = LongFloatHashMap(keys.size)
