...
open class IntLongHashMap(expectedElements: Int, loadFactor: Double = DEFAULT_LOAD_FACTOR.toDouble()) :
    Iterable<IntLongHashMap.IntLongCursor>, Accountable, Cloneable<IntLongHashMap> {
    ...
    constructor(map: IntLongHashMap) : this(map.size()) {
        putAll(map)
    }
    ...
    fun put(key: Int, value: Long): Long {
        ...
        val mask = this.mask
        if (((key) == 0)) {
            val previousValue = if (hasEmptyKey) values!![mask + 1] else 0L
            hasEmptyKey = true
            values!![mask + 1] = value
    ...
    fun putAll(iterable: Iterable<IntLongCursor>): Int {
        val count = size()
        for (c in iterable) {
            put(c.key, c.value)
        }
    ...
    protected fun equalElements(other: IntLongHashMap): Boolean {
        ...
        for (c in other) {
            val key = c.key
            if (!containsKey(key) || (get(key).hashCode()) != (c.value).hashCode()) {
                return false
            }
        }
    ...
    private inner class EntryIterator : AbstractIterator<IntLongCursor?>() {
        private val cursor: IntLongCursor
        ...
        override fun fetch(): IntLongCursor? {
            val mask = this@IntLongHashMap.mask
            while (index <= mask) {
                val existing: Int
                index++
    ...
    override fun iterator(): Iterator<IntLongCursor> {
        return EntryIterator() as Iterator<IntLongCursor>
    }
    ...
    inner class KeysContainer : Iterable<IntCursor> {
        ...
        fun size(): Int {
            return this@IntLongHashMap.size()
        }
     ...
     */
    fun values(): LongContainer {
        return ValuesContainer()
    }

    /** A view over the set of values of this map.  */
    private inner class ValuesContainer : LongContainer() {
        override fun iterator(): Iterator<LongCursor> {
            return ValuesIterator() as Iterator<LongCursor>
        }
    }
    ...
    private inner class ValuesIterator : AbstractIterator<LongCursor?>() {
    ...
    override fun clone(): IntLongHashMap {
        val cloned = IntLongHashMap()
    ...
    companion object {
        private val BASE_RAM_BYTES_USED: Long = RamUsageEstimator.shallowSizeOfInstance(IntLongHashMap::class)
        ...
        fun from(keys: IntArray, values: LongArray): IntLongHashMap {
            ...
            val map = IntLongHashMap(keys.size)
