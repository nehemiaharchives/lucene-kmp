...
open class IntFloatHashMap(expectedElements: Int, loadFactor: Float = DEFAULT_LOAD_FACTOR.toFloat()) :
    Iterable<IntFloatHashMap.IntFloatCursor>, Accountable, Cloneable<IntFloatHashMap> {
    ...
    constructor(map: IntFloatHashMap) : this(map.size()) {
        putAll(map)
    }

    fun put(key: Int, value: Float): Float {
    ...
    fun putAll(iterable: Iterable<IntFloatCursor>): Int {
        val count = size()
        for (c in iterable) {
            put(c.key, c.value)
        }
     ...
     * in the map.
     */
    fun putIfAbsent(key: Int, value: Float): Boolean {
     ...
     */
    fun putOrAdd(key: Int, putValue: Float, incrementValue: Float): Float {
     ...
     * @return Returns the current value associated with `key` (after changes).
     */
    fun addTo(key: Int, incrementValue: Float): Float {
        return putOrAdd(key, incrementValue, incrementValue)
    }

    fun remove(key: Int): Float {
        val mask = this.mask
        if (((key) == 0)) {
            if (!hasEmptyKey) {
                return 0f
    ...
    fun getOrDefault(key: Int, defaultValue: Float): Float {
        if (((key) == 0)) {
    ...
    fun indexInsert(index: Int, key: Int, value: Float) {
        var index = index
    ...
    protected fun equalElements(other: IntFloatHashMap): Boolean {
        if (other.size() != size()) {
            return false
        }
    ...
    private inner class EntryIterator : AbstractIterator<IntFloatCursor?>() {
        private val cursor: IntFloatCursor
        ...
        override fun fetch(): IntFloatCursor? {
            val mask = this@IntFloatHashMap.mask
            while (index <= mask) {
                val existing: Int
                index++
    ...
    override fun iterator(): Iterator<IntFloatCursor> {
        return EntryIterator() as Iterator<IntFloatCursor>
    }
    ...
    inner class KeysContainer : Iterable<IntCursor> {
        ...
        fun size(): Int {
            return this@IntFloatHashMap.size()
        }
    ...
    override fun clone(): IntFloatHashMap {
        val cloned = IntFloatHashMap()
    ...
    protected fun rehash(fromKeys: IntArray, fromValues: FloatArray) {
     ...
     */
    protected fun allocateThenInsertThenRehash(slot: Int, pendingKey: Int, pendingValue: Float) {
    ...
    companion object {
        private val BASE_RAM_BYTES_USED: Long = RamUsageEstimator.shallowSizeOfInstance(IntFloatHashMap::class)

        /** Creates a hash map from two index-aligned arrays of key-value pairs.  */
        fun from(keys: IntArray, values: FloatArray): IntFloatHashMap {
            require(keys.size == values.size) { "Arrays of keys and values must have an identical length." }

            val map = IntFloatHashMap(keys.size)
