...
open class IntDoubleHashMap(expectedElements: Int, loadFactor: Double = DEFAULT_LOAD_FACTOR.toDouble()) :
    Iterable<IntDoubleHashMap.IntDoubleCursor>, Accountable, Cloneable<IntDoubleHashMap> {
    ...
    constructor(map: IntDoubleHashMap) : this(map.size()) {
        putAll(map)
    }
    ...
    fun put(key: Int, value: Double): Double {
        ...
        val mask = this.mask
        if (((key) == 0)) {
            val previousValue = if (hasEmptyKey) values!![mask + 1] else 0.0
            hasEmptyKey = true
            values!![mask + 1] = value
    ...
    fun putAll(iterable: Iterable<IntDoubleCursor>): Int {
        val count = size()
        for (c in iterable) {
            put(c.key, c.value)
        }
     ...
     * in the map.
     */
    fun putIfAbsent(key: Int, value: Double): Boolean {
     ...
     */
    fun putOrAdd(key: Int, putValue: Double, incrementValue: Double): Double {
    ...
    fun get(key: Int): Double {
        if (((key) == 0)) {
            return if (hasEmptyKey) values!![mask + 1] else 0.0
        } else {
            val keys = this.keys
            val mask = this.mask
            var slot = hashKey(key) and mask
    ...
    fun indexReplace(index: Int, newValue: Double): Double {
        require(index >= 0) { "The index must point at an existing key." }
        require(index <= mask || (index == mask + 1 && hasEmptyKey))
    ...
    fun indexInsert(index: Int, key: Int, value: Double) {
        var index = index
    ...
    fun indexRemove(index: Int): Double {
        require(index >= 0) { "The index must point at an existing key." }
        require(index <= mask || (index == mask + 1 && hasEmptyKey))
    ...
    protected fun equalElements(other: IntDoubleHashMap): Boolean {
        if (other.size() != size()) {
            return false
        }
    ...
    private inner class EntryIterator : AbstractIterator<IntDoubleCursor?>() {
        private val cursor: IntDoubleCursor
        ...
        override fun fetch(): IntDoubleCursor? {
            val mask = this@IntDoubleHashMap.mask
            while (index <= mask) {
                val existing: Int
                index++
    ...
    override fun iterator(): Iterator<IntDoubleCursor> {
        return EntryIterator() as Iterator<IntDoubleCursor>
    }
    ...
    inner class KeysContainer : Iterable<IntCursor> {
        ...
        fun size(): Int {
            return this@IntDoubleHashMap.size()
        }
     ...
     */
    fun values(): DoubleContainer {
        return ValuesContainer()
    }

    /** A view over the set of values of this map.  */
    private inner class ValuesContainer : DoubleContainer() {
        override fun iterator(): Iterator<DoubleCursor> {
            return ValuesIterator() as Iterator<DoubleCursor>
        }
    }

    /** DoubleCursor iterable with size and toArray function implemented  */
    abstract inner class DoubleContainer : Iterable<DoubleCursor> {
        fun size(): Int {
            return this@IntDoubleHashMap.size()
        }
    ...
    private inner class ValuesIterator : AbstractIterator<DoubleCursor?>() {
        private val cursor: DoubleCursor
        private val increment: Int
        private var index = 0
        private var slot: Int
        ...
        override fun fetch(): DoubleCursor? {
            val mask = this@IntDoubleHashMap.mask
            while (index <= mask) {
                index++
                slot = (slot + increment) and mask
    ...
    override fun clone(): IntDoubleHashMap {
        val cloned = IntDoubleHashMap()
    ...
    protected fun rehash(fromKeys: IntArray, fromValues: DoubleArray) {
    ...
    protected fun allocateBuffers(arraySize: Int) {
        ...
        try {
            ...
            this.values = (DoubleArray(arraySize + emptyElementSlot))
        } catch (e: Error) {
            this.keys = prevKeys
     ...
     */
    protected fun allocateThenInsertThenRehash(slot: Int, pendingKey: Int, pendingValue: Double) {
    ...
    companion object {
        private val BASE_RAM_BYTES_USED: Long = RamUsageEstimator.shallowSizeOfInstance(IntDoubleHashMap::class)

        /** Creates a hash map from two index-aligned arrays of key-value pairs.  */
        fun from(keys: IntArray, values: DoubleArray): IntDoubleHashMap {
            require(keys.size == values.size) { "Arrays of keys and values must have an identical length." }

            val map = IntDoubleHashMap(keys.size)
