 ...
 * objects instead.
 */
class IndexSearcher(context: IndexReaderContext, executor: Executor?) {
    ...
    protected val leafContexts: MutableList<LeafReaderContext>
    ...
    private var queryCache: QueryCache = DEFAULT_QUERY_CACHE
    private var queryCachingPolicy: QueryCachingPolicy = DEFAULT_CACHING_POLICY
    ...
    constructor(r: IndexReader) : this(r, null)
     ...
     * @lucene.experimental
     */
    constructor(r: IndexReader, executor: Executor?) : this(r.context, executor)
    ...
    init {
        ...
        this.taskExecutor =
            if (executor == null) TaskExecutor({ obj: () -> Unit -> obj() }) else TaskExecutor(executor)
        this.readerContext = context
        leafContexts = context.leaves()
        if (executor == null) {
            leafSlices =
                if (leafContexts.isEmpty())
                    emptyArray<LeafSlice>()
                else
                    arrayOf<LeafSlice>(
                        LeafSlice(
                            ArrayList<LeafReaderContextPartition>(
                                leafContexts
                                    .map { ctx: LeafReaderContext ->
                                        LeafReaderContextPartition.Companion.createForEntireSegment(
                                            ctx
                                        )
                                    }
                                    .toList()))
     ...
     * @lucene.experimental
     */
    constructor(context: IndexReaderContext) : this(context, null)
    ...
    fun setQueryCachingPolicy(queryCachingPolicy: QueryCachingPolicy) {
        this.queryCachingPolicy = requireNotNull<QueryCachingPolicy>(queryCachingPolicy)
    }
    ...
    fun count(query: Query): Int {
        ...
        if (query is BooleanQuery
            && this.reader.hasDeletions() === false && query.isTwoClausePureDisjunctionWithTerms()
        ) {
            val queries: Array<Query> = query.rewriteTwoClauseDisjunctionWithTermsForCount(this)
            ...
            } else if (min(countTerm1, countTerm2) as Double / max(countTerm1, countTerm2)
                < 0.1
            ) {
                return countTerm1 + countTerm2 - count(queries[2])
            }
        }
        return search(ConstantScoreQuery(query), TotalHitCountCollectorManager(this.slices))
    }
     ...
     */
    @Throws(IOException::class)
    fun searchAfter(after: ScoreDoc, query: Query, numHits: Int): TopDocs {
        val limit: Int = kotlin.math.max(1, reader.maxDoc())
        ...
        return search<Collector, TopDocs>(query, manager)
    }
    ...
    fun setTimeout(queryTimeout: QueryTimeout) {
   ...
   *     CollectorManager)} due to its support for concurrency in IndexSearcher"""
    ...
    fun search(query: Query, collector: Collector) {
        ...
        val weight = createWeight(query, collector.scoreMode(), 1f)
     ...
     * @param sort The [org.apache.lucene.search.Sort] object
     * @return The top docs, sorted according to the supplied [org.apache.lucene.search.Sort]
     * instance
     * @throws IOException if there is a low-level I/O error
     */
    @Throws(IOException::class)
    fun search(query: Query, n: Int, sort: Sort): TopFieldDocs {
    ...
    @Throws(IOException::class)
    fun searchAfter(
        after: ScoreDoc, query: Query, numHits: Int, sort: Sort, doDocScores: Boolean
    ): TopFieldDocs {
    ...
    private fun searchAfter(
        after: FieldDoc, query: Query, numHits: Int, sort: Sort, doDocScores: Boolean
    ): TopFieldDocs {
        val limit: Int = kotlin.math.max(1, reader.maxDoc())
        require(!(after != null && after.doc >= limit)) {
            ("after.doc exceeds the number of documents in the reader: after.doc="
        ...
        val topDocs: TopFieldDocs = search<Collector, TopFieldDocs>(query, manager)
        if (doDocScores) {
     ...
     */
    @Throws(IOException::class)
    fun <C : Collector, T> search(query: Query, collectorManager: CollectorManager<C, T>): T {
        var query = query
        val firstCollector: C = collectorManager.newCollector()
        query = rewrite(query, firstCollector.scoreMode().needsScores())
        val weight = createWeight(query, firstCollector.scoreMode(), 1f)
        return search(weight, collectorManager, firstCollector)
    }

    @Throws(IOException::class)
    private fun <C : Collector, T> search(
        weight: Weight, collectorManager: CollectorManager<C, T>, firstCollector: C
    ): T {
        ...
        if (leafSlices.size == 0) {
            ...
            require(leafContexts.isEmpty())
            return collectorManager.reduce(mutableListOf<T>(firstCollector))
        } else {
            val collectors: MutableList<C> = java.util.ArrayList<C>(leafSlices.size)
            ...
            val listTasks: MutableList<java.util.concurrent.Callable<C>> =
                java.util.ArrayList<java.util.concurrent.Callable<C>>(leafSlices.size)
            for (i in leafSlices.indices) {
                val leaves: Array<LeafReaderContextPartition> = leafSlices[i].partitions
                val collector = collectors.get(i)
                listTasks.add(
            ...
            val results: MutableList<C> = taskExecutor.invokeAll(listTasks)
     ...
     */
    @Throws(IOException::class)
    protected fun search(partitions: Array<LeafReaderContextPartition>, weight: Weight, collector: Collector) {
        collector.setWeight(weight)
    ...
    protected fun searchLeaf(
        ctx: LeafReaderContext, minDocId: Int, maxDocId: Int, weight: Weight, collector: Collector
    ) {
        ...
        if (scorerSupplier != null) {
            ...
            try {
                ...
                val acceptDocs: Bits = ScorerUtil.likelyLiveDocs(ctx.reader().getLiveDocs())
    ...
    @Throws(IOException::class)
    private fun rewrite(original: Query, needsScores: Boolean): Query {
        if (needsScores) {
            return rewrite(original)
        } else {
            // Take advantage of the few extra rewrite rules of ConstantScoreQuery.
            return rewrite(ConstantScoreQuery(original))
        }
    }
    ...
    protected fun explain(weight: Weight, doc: Int): Explanation {
        ...
        val liveDocs: Bits = ctx.reader().getLiveDocs()
    ...
    class LeafSlice(leafReaderContextPartitions: MutableList<LeafReaderContextPartition>) {
        ...
        init {
            val docBaseComparator: java.util.Comparator<LeafReaderContextPartition> =
                java.util.Comparator.comparingInt<LeafReaderContextPartition>(java.util.function.ToIntFunction { l: LeafReaderContextPartition -> l!!.ctx.docBase })
            val minDocIdComparator: java.util.Comparator<LeafReaderContextPartition> =
                java.util.Comparator.comparingInt<LeafReaderContextPartition>(java.util.function.ToIntFunction { l: LeafReaderContextPartition -> l!!.minDocId })
            leafReaderContextPartitions.sort(docBaseComparator.thenComparing(minDocIdComparator))
            this.partitions = leafReaderContextPartitions.toTypedArray<LeafReaderContextPartition>()
            this.maxDocs =
                java.util.Arrays.stream<LeafReaderContextPartition>(partitions)
                    .map<Int> { leafPartition: LeafReaderContextPartition -> leafPartition.maxDocs }
                    .reduce { a: Int, b: Int -> java.lang.Integer.sum(a, b) }
                    .get()
        }
    }
    ...
    class LeafReaderContextPartition private constructor(
        ...
        companion object {
            ...
            fun createFromAndTo(
                ...
                require(maxDocId != DocIdSetIterator.NO_MORE_DOCS)
    ...
    fun collectionStatistics(field: String): CollectionStatistics {
        ...
        return CollectionStatistics(field, reader.maxDoc(), docCount, sumTotalTermFreq, sumDocFreq)
    }
    ...
    companion object {
        ...
        private var DEFAULT_CACHING_POLICY: QueryCachingPolicy = UsageTrackingQueryCachingPolicy()

        init {
            val maxCachedQueries = 1000
            // min of 32MB or 5% of the heap size
            val maxRamBytesUsed = min(1L shl 25, java.lang.Runtime.getRuntime().maxMemory() / 20)
        ...
        private val defaultSimilarity: Similarity = BM25Similarity()
        ...
        fun setMaxClauseCount(value: Int) {
        ...
        fun slices(
            ...
            val sortedLeaves: MutableList<LeafReaderContext> = java.util.ArrayList<LeafReaderContext>(leaves)

            // Sort by maxDoc, descending:
            sortedLeaves.sort(
                java.util.Collections.reverseOrder<LeafReaderContext>(
                    java.util.Comparator.comparingInt<LeafReaderContext>(
                        java.util.function.ToIntFunction { l: LeafReaderContext -> l.reader().maxDoc() })
                )
            )

            if (allowSegmentPartitions) {
                val groupedLeafPartitions: MutableList<MutableList<LeafReaderContextPartition>> =
                    java.util.ArrayList<MutableList<LeafReaderContextPartition>>()
                ...
                for (ctx in sortedLeaves) {
                    if (ctx.reader().maxDoc() > maxDocsPerSlice) {
                        ...
                        // equal size
                        val numSlices: Int =
                            java.lang.Math.min(5, java.lang.Math.ceilDiv(ctx.reader().maxDoc(), maxDocsPerSlice))
                        val numDocs: Int = ctx.reader().maxDoc() / numSlices
                        var maxDocId = numDocs
                        var minDocId = 0
                        for (i in 0..<numSlices - 1) {
                            groupedLeafPartitions.add(
                                mutableListOf<LeafReaderContextPartition>(
                                    LeafReaderContextPartition.Companion.createFromAndTo(ctx, minDocId, maxDocId)
                                )
                            )
                            minDocId = maxDocId
                            maxDocId += numDocs
                        ...
                        groupedLeafPartitions.add(
                            mutableListOf<LeafReaderContextPartition>(
                                LeafReaderContextPartition.Companion.createFromAndTo(
                        ...
                        if (group == null) {
                            group = java.util.ArrayList<LeafReaderContextPartition>()
                ...
                val slices: Array<LeafSlice> = kotlin.arrayOfNulls<LeafSlice>(groupedLeafPartitions.size)
                var upto = 0
            ...
            val groupedLeaves: MutableList<MutableList<LeafReaderContext>> =
                java.util.ArrayList<MutableList<LeafReaderContext>>()
            var docSum: Long = 0
            var group: MutableList<LeafReaderContext> = null
            for (ctx in sortedLeaves) {
                if (ctx.reader().maxDoc() > maxDocsPerSlice) {
                    require(group == null)
                    groupedLeaves.add(mutableListOf<LeafReaderContext>(ctx))
                } else {
                    if (group == null) {
                        group = java.util.ArrayList<LeafReaderContext>()
                        group!!.add(ctx)
            ...
            val slices: Array<LeafSlice> = kotlin.arrayOfNulls<LeafSlice>(groupedLeaves.size)
            var upto = 0
            for (currentLeaf in groupedLeaves) {
                slices[upto] =
                    LeafSlice(
                        java.util.ArrayList<LeafReaderContextPartition>(
                            currentLeaf.stream()
                                .map<LeafReaderContextPartition> { ctx: LeafReaderContext ->
                                    LeafReaderContextPartition.Companion.createForEntireSegment(
                                        ctx
                                    )
                                }
        ...
        private fun enforceDistinctLeaves(leafSlice: LeafSlice) {
            val distinctLeaves: MutableSet<LeafReaderContext> = java.util.HashSet<LeafReaderContext>()
            for (leafPartition in leafSlice.partitions) {
                check(distinctLeaves.add(leafPartition.ctx) != false) { "The same slice targets multiple leaf partitions of the same leaf reader context. A physical segment should rather get partitioned to be searched concurrently from as many slices as the number of leaf partitions it is split into." }
            }
        }
        ...
        private val numClausesCheckVisitor: QueryVisitor
            ...
            get() = object : QueryVisitor() {
                ...
                public override fun getSubVisitor(occur: Occur, parent: Query): QueryVisitor {
                    // Return this instance even for MUST_NOT and not an empty QueryVisitor
                    return this
                }

                public override fun visitLeaf(query: Query) {
                ...
                public override fun consumeTerms(query: Query, vararg terms: Term) {
                    if (numClauses > maxClauseCount) {
                        throw TooManyNestedClauses()
                    }
                    ++numClauses
                ...
                public override fun consumeTermsMatching(
                    query: Query, field: String, automaton: java.util.function.Supplier<ByteRunAutomaton>
                ) {
                    if (numClauses > maxClauseCount) {
                        throw TooManyNestedClauses()
                    }
