...
class TestTrackingDirectoryWrapper : BaseDirectoryTestCase() {

    @Throws(Exception::class)
    override fun getDirectory(path: Path): Directory {
        return TrackingDirectoryWrapper(ByteBuffersDirectory())
    }

    @Test
    fun testTrackEmpty() {
        val dir = TrackingDirectoryWrapper(ByteBuffersDirectory())
        assertEquals(emptySet<String>(), dir.createdFiles)
    }
    ...
    fun testTrackCreate() {
        ...
        assertEquals(setOf("foo"), dir.createdFiles)
    }
    ...
    fun testTrackRename() {
        ...
        assertEquals(setOf("foo"), dir.createdFiles)
        dir.rename("foo", "bar")
        assertEquals(setOf("bar"), dir.createdFiles)
    }

    @Test
    fun testTrackCopyFrom() {
        val source = TrackingDirectoryWrapper(ByteBuffersDirectory())
        val dest = TrackingDirectoryWrapper(ByteBuffersDirectory())
        source.createOutput("foo", newIOContext(random())).close()
        assertEquals(setOf("foo"), source.createdFiles)
        dest.copyFrom(source, "foo", "bar", newIOContext(random()))
        assertEquals(setOf("bar"), dest.createdFiles)
        assertEquals(setOf("foo"), source.createdFiles)
    }
