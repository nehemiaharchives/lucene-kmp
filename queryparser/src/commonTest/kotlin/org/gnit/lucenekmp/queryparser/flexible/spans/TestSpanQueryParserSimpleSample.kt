 ...
 */
class TestSpanQueryParserSimpleSample : LuceneTestCase() {

    @Test
    fun testBasicDemo() {
        val queryParser: SyntaxParser = StandardSyntaxParser()

        // convert the CharSequence into a QueryNode tree
        var queryTree: QueryNode = queryParser.parse("body:text", null)

        // create a config handler with a attribute used in
        // UniqueFieldQueryNodeProcessor
        val spanQueryConfigHandler: QueryConfigHandler = SpansQueryConfigHandler()
        ...
        val spanProcessorPipeline = QueryNodeProcessorPipeline(spanQueryConfigHandler)
        // @see SpansValidatorQueryNodeProcessor
        spanProcessorPipeline.add(SpansValidatorQueryNodeProcessor())
        // @see UniqueFieldQueryNodeProcessor
        spanProcessorPipeline.add(UniqueFieldQueryNodeProcessor())

        // print to show out the QueryNode tree before being processed
        if (VERBOSE) println(queryTree)
        ...
        val spansQueryTreeBuilder = SpansQueryTreeBuilder()

        // convert QueryNode tree to span query Objects
        val spanquery: SpanQuery = spansQueryTreeBuilder.build(queryTree)
