...
/**
 * Builds no object, it only returns the [Query] object set on the [GroupQueryNode] object using a
 * [QueryTreeBuilder.QUERY_TREE_BUILDER_TAGID] tag.
 */
class GroupQueryNodeBuilder : StandardQueryBuilder {
    init {
        // empty constructor
    }

    override fun build(queryNode: QueryNode): Query {
        val groupNode = queryNode as GroupQueryNode

        return groupNode.child.getTag(QueryTreeBuilder.QUERY_TREE_BUILDER_TAGID) as Query
    }
}
