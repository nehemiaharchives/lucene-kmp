...
/**
 * A [BooleanModifierNode] has the same behaviour as [ModifierQueryNode], it only indicates that
 * this modifier was added by BooleanQuery2ModifierNodeProcessor and not by the user.
 *
 * @see ModifierQueryNode
 */
class BooleanModifierNode(node: QueryNode, mod: Modifier) : ModifierQueryNode(node, mod) {
    override fun cloneNode(): QueryNodeImpl {
        return BooleanModifierNode(child.cloneTree(), modifier)
    }
}
