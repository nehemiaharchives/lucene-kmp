...
/**
 * Builds no object, it only returns the [Query] object set on the [ModifierQueryNode] object using
 * a [QueryTreeBuilder.QUERY_TREE_BUILDER_TAGID] tag.
 */
class ModifierQueryNodeBuilder : StandardQueryBuilder {
    init {
        // empty constructor
    }

    override fun build(queryNode: QueryNode): Query {
        val modifierNode = queryNode as ModifierQueryNode

        return modifierNode.child.getTag(QueryTreeBuilder.QUERY_TREE_BUILDER_TAGID) as Query
    }
}
