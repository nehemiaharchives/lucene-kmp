...
open class AndQueryNode(
    /** @param clauses - the query nodes to be and'ed */
    clauses: List<QueryNode>,
) : BooleanQueryNode(clauses) {
    init {
        if (clauses.isEmpty()) {
            throw IllegalArgumentException("AND query must have at least one clause")
        }
    }
    ...
    override fun cloneNode(): QueryNodeImpl {
        return AndQueryNode(requireNotNull(children).map { it.cloneTree() })
    }
}
