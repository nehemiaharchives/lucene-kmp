...
class PrefixWildcardQueryNodeBuilder : StandardQueryBuilder {
    init {
        // empty constructor
    }

    override fun build(queryNode: QueryNode): PrefixQuery {
        val wildcardNode = queryNode as PrefixWildcardQueryNode
        ...
        val wildcardText = requireNotNull(wildcardNode.text)
        val text = wildcardText.subSequence(0, wildcardText.length - 1).toString()

        return PrefixQuery(Term(requireNotNull(wildcardNode.getFieldAsString()), text), method)
    }
}
