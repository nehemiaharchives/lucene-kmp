...
Style 6. Nullability: Get advantage of the kotlin language of null handling as much as possible by trying to use none-nullable var, val as much as possible. if a java lucene specifically inserts `null` to the val/var, then it can be nullable to mimic the behavior. If the value is kotlin-none-nullable, `Objects.requireNonNull()` can be omitted but if it is nullable in kotlin, use top level function `requireNotNull()`. 
