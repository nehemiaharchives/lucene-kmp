...
2. **After any file change, immediately run `open_file_in_editor` first, then run `get_file_problems` for each changed file and fix all compile errors before proceeding.**
2. Use `open_file_in_editor` tool of `jetbrains` MCP server first, then `get_file_problems` to check if there are any compilation errors for each files you changed. `get_file_problems` may not emit diagnostics unless the file is opened in the editor first. iterate over until you solve all errors for Kotiln/JVM.
