...
class MockTokenizer : Tokenizer {
    ...
    private val random = Random(LuceneTestCase.Companion.random().nextLong())
    ...
    override fun incrementToken(): Boolean {
        ...
        while (true) {
            ...
            if (cp < 0) {
                ...
                var endOffset: Int
                do {
                    val len = Character.Companion.toChars(normalize(cp), chars, 0)
    ...
    protected open fun normalize(c: Int): Int = if (lowerCase) Character.Companion.toLowerCase(c) else c
